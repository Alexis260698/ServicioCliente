...
public class ClienteController {
    ...
    public ResponseEntity<Cliente> buscarCliente(@PathVariable("id") String usuario){
        ...
        if(cliente.isEmpty()){
            ...
            return ResponseEntity.ok(clienteService.mostrarTarjetas(cliente.get()));
        }
    }
    ...
    @PutMapping("/agregarPlazoFijo/{usuario}")
    public ResponseEntity<Cliente> addPlazoFijo(@PathVariable("usuario") String usuario, @RequestBody PlazoFijo plazoFijo) {
        System.out.println(usuario);
        System.out.println(plazoFijo.getPlazoDias());
        Optional<Cliente> cliente = clienteService.addPlazoFijo(usuario, plazoFijo);
        if (cliente.isEmpty()) {
            return ResponseEntity.notFound().build();
        } else {
            return ResponseEntity.ok(cliente.get());
