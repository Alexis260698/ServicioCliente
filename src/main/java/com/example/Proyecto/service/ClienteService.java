...
public class ClienteService {
    ...
    public Optional<Cliente> addPlazoFijo(String usuario, PlazoFijo plazoFijo) {
        Optional<Cliente> cliente= buscarClientePorUsuario(usuario);
        if(cliente.isPresent()){
            List<PlazoFijo> listaPlazofijo=cliente.get().getPlazoFijoLista();
            listaPlazofijo.add(plazoFijo);
            cliente.get().setPlazoFijoLista(listaPlazofijo);
            clienteRepositoryDao.save(cliente.get());
        }
        return cliente;
    }

    public void actualizarCliente(Cliente cliente) {
        clienteRepository.actualizaCliente(cliente);
    }


    public Cliente mostrarTarjetas(Cliente cliente) {
    List<TarjetaDebito> tarjetaDebitoList= new ArrayList<>();

    for (Cuenta c: cliente.getCuentas()){
        if(c.getTarjetaDebito()!=null){
            tarjetaDebitoList.add(c.getTarjetaDebito());
        }
    }

    cliente.setTarjetas(tarjetaDebitoList);
    return cliente;
    }
}
